...
public class incomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        input_detail = findViewById(R.id.input_detail_income_edittext);
        input_money = findViewById(R.id.input_money_income_edittext);
        save_button = findViewById(R.id.save_income_button);

        save_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(input_detail.getText().toString().equalsIgnoreCase("")||input_money.getText().toString().equalsIgnoreCase("")){
                    Toast.makeText(incomeActivity.this, "กรุณาใส่ข้อมูลให้ครบ", Toast.LENGTH_SHORT).show();
                }else{
                    savedatatodb();
                    setResult(RESULT_OK);
                    finish();
